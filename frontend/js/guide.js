...
The <strong style="color:var(--teal)">Execution Engine</strong> receives PMO Directives, routes action items to the 15 SHIFT LangGraph agents, emits structured Execution Result payloads back to iOPEX PMO Engine. Each result is ingested into the RAG corpus, closing the weekly PMO cycle.
  ...
  _guideSection('&#x1F512; Security Deep Dive — Q&amp;A for Cisco Security Architect', `
    ...
    ${[
      ...
      ['Network Security', [
        ...
        ['How are secrets injected into the CI/CD pipeline without hardcoding?',
          'Secrets are injected at runtime from the chosen secret store (Keeper KSM). They are never in YAML, never in environment variable blocks in pipeline config files, never in logs. <code>trufflehog</code> and <code>detect-secrets</code> scan every commit and every file before merge — the pipeline hard-fails on any credential pattern detected. This is non-negotiable for a system handling live privileged credentials.'],
      ]],
      ['Audit, Compliance &amp; Governance', [
        ['What happens to the CyberArk audit trail when we migrate?',
          '<strong>This is the most critical compliance risk in the program.</strong> CyberArk audit history does NOT migrate to any target platform. The audit trail restarts at the migration date. Agent 07 automatically exports and archives all CyberArk audit logs before decommission in P7. Archived logs must be retained per Cisco\'s compliance retention policy (7 years for SOX, 1 year for PCI-DSS minimum). The archive must be accessible to auditors independently of the new platform.'],
        ['How do we maintain PCI-DSS compliance through the migration window?',
          'Three controls:<br><br><strong>1. Dual-control at gate g3</strong> — Agent 03 permission mapping output reviewed by Cisco Security Architect, not iOPEX.<br><strong>2. Parallel running (P6)</strong> — both CyberArk and the target are live simultaneously; no single point of failure during transition.<br><strong>3. Read-only gate (g13)</strong> — CyberArk is set to read-only (not decommissioned) until all three independent Cisco approvers (CAB, Exec Sponsor, Compliance) authorize.'],
        ...
        ['What is the highest-authority gate and what can block it?',
          'Gate g13 — Cutover Approval. This sets the source CyberArk vault to read-only, the last reversible action before decommission. iOPEX presents evidence: parallel-run metrics, final validation report, all App Owner sign-offs, and compliance packages. All three Cisco approvers must authorize. If any approver is unavailable or withholds approval, the migration pauses indefinitely. iOPEX cannot proceed on partial or verbal authorization.'],
        ['How is the audit log itself protected against tampering?',
          'The orchestrator writes audit logs in JSONL format with a SHA-256 hash chain — each entry includes the hash of the previous entry. Any tampering with a log entry breaks the chain and is immediately detectable. This is SIEM-ready format for ingestion into Cisco\'s existing SIEM infrastructure.'],
      ]],
      ['Non-Human Identities (NHI) &amp; DevOps Secrets', [
        ...
        ['What replaces CCP/AAM for application credential retrieval?',
          'Keeper Secrets Manager (KSM) — 40+ native integrations covering GitHub Actions, Terraform, Ansible, Kubernetes, Jenkins. Applications that used CCP REST API or AAM agent get re-pointed to KSM SDK calls. Agent 06 scans all application code for CCP/AAM patterns and generates the replacement code. This is a full re-architecture for any app that relied on the AAM agent injection model.'],
        ['What about Conjur secrets for Kubernetes and CI/CD pipelines?',
          'Conjur secrets require platform re-architecture — app teams must update their SDK calls. This is out-of-scope for the automated migration and requires direct engagement with each app team. These are tracked as PENDING_EXTERNAL_TEAM items in the PMO Directive.'],
        ['Are there zero standing privilege controls in the target architecture?',
          'Yes — Devolutions RDM entries pull credentials directly from Keeper via KSM API at session launch. Passwords are never stored in RDM. They are retrieved just-in-time and discarded after the session closes. Combined with MiniOrange\'s adaptive authentication, no user holds a standing credential to a privileged system outside of an active, logged session.'],
      ]],
      ['Penetration Testing &amp; Security Validation', [
        ['When does penetration testing occur and what does it cover?',
          'Pre-P5 (before production waves begin). Scope:<br><br>• <strong>Secret zeroing validation</strong> — confirm credentials are not recoverable from memory after use<br>• <strong>State file permissions</strong> — confirm migration state files are not world-readable<br>• <strong>TLS configuration</strong> — confirm cipher suites, certificate validation, no self-signed certs in production paths<br>• <strong>API authentication posture</strong> — confirm no unauthenticated endpoints, token scoping, rotation<br>• <strong>Injection vectors</strong> — subprocess calls, config parsing, error message sanitization'],
        ['What static analysis runs on the orchestrator code itself?',
          'Two layers:<br><br>• <code>bandit</code> — Python security anti-patterns: hardcoded secrets, subprocess injection, insecure deserialization. Pipeline hard-fails on any HIGH severity finding.<br>• <code>trufflehog</code> / <code>detect-secrets</code> — scans repo history and staged files for credential patterns. A single match hard-fails the pipeline and blocks merge until remediated.'],
      ]],
      ['Operational Security During Migration', [
        ...
        ['What is the rollback posture if a production wave fails?',
          'The source CyberArk vault remains fully operational until gate g13. At any point before g13, the migration is fully reversible — unfreeze the source, invalidate target credentials, restore from last-known-good state backup. After g13 (read-only), rollback requires Vault DR promotion and is a major incident. The gate structure is specifically designed to make g13 the only point of no return.'],
        ['How is the migration service account itself secured and monitored?',
          'The <code>svc-migration</code> service account has scoped access only to Safes actively being migrated in the current wave — not all Safes simultaneously. After each wave, its Safe membership is trimmed. CyberArk PTA monitors the account for anomalous activity patterns throughout the migration. Any access outside normal migration hours or to out-of-scope Safes fires a PTA alert.'],
      ]],
    ].map(([category, qas]) => `
      <div style="margin-bottom:20px;">
        <div style="font-size:0.65rem;font-family:var(--font-mono);color:var(--amber);text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid var(--border);">${category}</div>
        ${qas.map(([q, a]) => `
          <div style="margin-bottom:14px;padding:10px 14px;background:var(--bg-page);border-radius:6px;border-left:3px solid var(--blue);">
            <div style="font-weight:700;color:var(--text-bright);margin-bottom:6px;font-size:0.72rem;">Q: ${q}</div>
            <div style="color:var(--text-standard);font-size:0.7rem;line-height:1.7;">${a}</div>
          </div>
        `).join('')}
      </div>
    `).join('')}

    <div style="margin-top:16px;padding:12px 14px;background:var(--bg-surface);border-radius:6px;border:1px solid var(--amber);">
      <div style="font-size:0.65rem;font-family:var(--font-mono);color:var(--amber);text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px;">Security Risk Summary — Sign-Off Matrix</div>
      <table style="width:100%;border-collapse:collapse;font-size:0.65rem;">
        <thead>
          <tr style="color:var(--text-muted);text-transform:uppercase;letter-spacing:.04em;">
            <th style="text-align:left;padding:4px 8px;border-bottom:1px solid var(--border);">Risk</th>
            <th style="text-align:left;padding:4px 8px;border-bottom:1px solid var(--border);">Severity</th>
            <th style="text-align:left;padding:4px 8px;border-bottom:1px solid var(--border);">Owner</th>
            <th style="text-align:left;padding:4px 8px;border-bottom:1px solid var(--border);">Mitigation</th>
          </tr>
        </thead>
        <tbody>
          ${[
